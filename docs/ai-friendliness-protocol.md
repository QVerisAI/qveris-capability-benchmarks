...
| 输出自解释 | `agent_response_self_description` | 仅凭响应能否确定币对、数值单位、时间/时区 |
...
## 5. 输出自解释性：返回数据对 AI 是否自解释

响应里没有单位、币种、时间/时区时，AI 要么猜测（幻觉风险）要么解读不完整——这是
工具输出侧的 AI 友好度。`agent_response_self_description` 用使用方视角测量：对冻结的
真实响应逐项询问，AI 仅凭响应能否确定 {币对, 数值单位, 时间/时区} 三个语义要素。

- 判定：AI 回答"能"→ 该要素自解释；回答"不能"→ 记录缺失字段，该要素不自解释。
- 每个要素 2 轮；回答原文留档（防止"能/不能"误判）。
- 同一模型测所有供应商，差异归因于响应 schema（是否携带币种/单位/时区字段）。
- 输出自解释性不是能力扣分，是契约响应设计的可测量属性；缺失字段列入对供应商的
  改进建议。
