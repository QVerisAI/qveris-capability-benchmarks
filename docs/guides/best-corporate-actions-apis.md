...
- 重要说明：本文延迟与费用为 2026-08-08 经 QVeris 生产网关实测，不代表供应商直连指标；公司行动属于低频专业数据，多数供应商本快照样本量偏小，结论以复测为准。
...
- 实测：固定输入（AAPL 拆股/分红检索、无效代码负向控制），每个适用单元 2 轮，2026-08-08 经 QVeris 生产网关执行。
...
恒生聚源是本次测试中唯一完整达标的供应商：实测延迟 0.9 秒、每次调用 1 credit、A 股生产调用量最大。同花顺 iFinD 延迟更低（0.7 秒）且成本同为 1 credit，但本快照未完全达到公司行动契约的完整性门槛，适合在确认所需动作字段后使用。
