...
- A per-dimension score requires a typed metric registered by the release's CAP,
  a content-addressed definition, versioned method, explicit scale, Provider,
  Access Path, and released evidence. A rank additionally pins its frozen cohort
  and tie method; the release gate recomputes the cohort digest from its members.
