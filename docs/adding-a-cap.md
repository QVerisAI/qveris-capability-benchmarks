...
1. Export the private Harbor catalog and select a real contract from it. Do not
   create a CAP Pack from a local idea or an external benchmark.
...
6. Compile with `qveris-bench suite freeze cap_packs/<cap>/suite.yaml --harbor-contracts .harbor-snapshots/catalog/contracts.json`, then
