...
def extract_finnhub_stock_quote(
    ...
    if _is_unavailable_quote(data):
        raise StockQuoteExtractionError("quote is unavailable")
    price = _positive_number(data.get("c"), "price")
    timestamp = _timestamp(data.get("t"))
    facts: dict[str, Any] = {"symbol": symbol, "price": price, "timestamp": timestamp}
...
def extract_eodhd_stock_quote(
    ...
    if _eodhd_negative_response(document):
        raise StockQuoteExtractionError("quote is unavailable")
...
def _eodhd_negative_response(document: dict[str, Any]) -> bool:
    ...
    is_parameter_validation = (
        "return valid data" in message and "verify the request parameters" in message
    )
    if not (is_symbol_validation or is_invalid_parameter or is_parameter_validation):
        return False
    result = document.get("result")
    if not isinstance(result, dict):
        return False
