...
def _release(*facts: ReleaseFact) -> BenchmarkRelease:
    return BenchmarkRelease(
        ...
        evidence_ids=tuple(f"evidence-{index}" for index in range(1, 3)),
        developer_selection_facts=facts,
    )
...
def test_ac2_release_gate_rejects_inconsistent_frozen_cohort() -> None:
    ...
    with pytest.raises(ReleaseGateError, match="cohort is inconsistent"):
        validate_release_inputs(
